...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (!empty($nombreProducto) && !empty($marcaProducto) && !empty($categoriaProducto) && is_numeric($stockProducto) && is_numeric($precioProducto)) {
        // Preparar la consulta de inserción
        $consultaInsercion = "INSERT INTO productos (product_nombre, product_marca, product_categoria, product_stock, product_precio, product_descripcion) VALUES ('$nombreProducto', '$marcaProducto', '$categoriaProducto', $stockProducto, $precioProducto, '$descripcionProducto')";

        // Ejecutar la consulta
        if (mysqli_query($conexion, $consultaInsercion)) {
            $productoexitoso = "<p>Producto agregado exitosamente.</p>";
        } 
    } 
}
...
// Establecer conexión con la base de datos
$conexion = mysqli_connect("127.0.0.1:3306", "root", "", "productos administrador");

// Obtener el ID del producto a eliminar
if (isset($_GET['id'])) {
    $IDproducto = $_GET['id'];
} else {
    echo "No se encontró el ID del producto.";
    echo "<a href='6 - index - admin - productos.php'>Volver a la lista de productos</a>";
    exit();
}

// Eliminar el producto de la base de datos
$consultaEliminacion = "DELETE FROM productos WHERE product_id = $IDproducto";
$resultadoEliminacion = mysqli_query($conexion, $consultaEliminacion);


// Verificar si la eliminación fue exitosa
if ($resultadoEliminacion) {
    echo "<form action='' method='post' class='formulario_agregar' id='editarFormulario'>";
    echo "<h2>Eliminar producto</h2>";
    echo "<input type='text' name='nombreProducto' value='Producto ID: $IDproducto eliminado exitosamente.'>";
    echo "<a href='6 - index - admin - productos.php'>Volver a la lista de productos</a>";
} else {
    echo "<form action='' method='post' class='formulario_agregar' id='editarFormulario'>";
    echo "<h2>Eliminar producto</h2>";
    echo "<input type='text' name='nombreProducto' value='Error al eliminar el producto: ". mysqli_error($conexion) . "'>";
    echo "<a href='6 - index - admin - productos.php'>Volver a la lista de productos</a>";
