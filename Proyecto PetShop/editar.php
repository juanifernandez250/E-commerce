...
<header>
    <!-- MENU DE NAVEGACION -->
    <div class="logo">
        <a href="./1 - index-inicio.html"><img src="./assets/images/bg6.jpg" alt="logo"></a>
    </div>
    <nav>
        <div class="headermenu">
            <ul>
                <li><a href="./1 - index-inicio.html">Inicio</a></li>
                <li><a href="./2 - index-quienes-somos.html">¿Quiénes Somos?</a></li>
                <li><a href="./3 - index-Productos.html">Productos</a></li>
                <li><a href="./4 - index-Iniciar-Sesion.html">Ingresar</a></li>
                <li><a href="./5 - index-Carrito.html">Carrito</a></li>
            </ul>
        </div>
...
<body class="agregar_producto">
    
<?php
// Establecer conexión con la base de datos
$conexion = mysqli_connect("127.0.0.1", "root", "", "productos administrador");
...
if (isset($_GET['id'])) {
    ...
    echo "No se encontró ningún ID de producto en la URL.";
    exit();
}

// Recuperar los datos del producto
$consulta = "SELECT * FROM productos WHERE product_id = ?";
...
if ($resultado && mysqli_num_rows($resultado) > 0) {
    ...
    <div class='contenedor_agregar'>
        <form action="" method="post" class="formulario_agregar" id="editarFormulario">
            <h2>Editar de producto</h2>
            <input type="hidden" name="IDproducto" value="<?php echo $IDproducto; ?>">
            <input type="text" name="nombreProducto" value="<?php echo htmlspecialchars($datosProducto['product_nombre']); ?>" required>
            <input type="text" id="marcaProducto" name="marcaProducto" value="<?php echo htmlspecialchars($datosProducto['product_marca']); ?>" required>
            <select class='select_agregar' name='categoriaProducto' required>
                <option value='alimentos' <?php if ($datosProducto['product_categoria'] == 'alimentos') echo 'selected'; ?>>Alimentos</option>
                <option value='juguetes' <?php if ($datosProducto['product_categoria'] == 'juguetes') echo 'selected'; ?>>Juguetes</option>
                <option value='accesorios' <?php if ($datosProducto['product_categoria'] == 'accesorios') echo 'selected'; ?>>Accesorios</option>
                <option value='higiene' <?php if ($datosProducto['product_categoria'] == 'higiene') echo 'selected'; ?>>Higiene</option>
            </select>
            <input type='number' id='stockProducto' name='stockProducto' value='<?php echo htmlspecialchars($datosProducto['product_stock']); ?>' required>
            <input type='number' id='precioProducto' name='precioProducto' value='<?php echo htmlspecialchars($datosProducto['product_precio']); ?>' step='0.01' required>
            <input type='text' id='descripcionProducto' name='descripcionProducto' value='<?php echo htmlspecialchars($datosProducto['product_descripcion']); ?>' required>
...
// Actualizar los datos del producto si se envía el formulario
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['IDproducto'])) {
    $IDproductoActualizado = $_POST['IDproducto'];
    $nombreProductoActualizado = $_POST['nombreProducto'];
    $marcaProductoActualizada = $_POST['marcaProducto'];
    $categoriaProductoActualizada = $_POST['categoriaProducto'];
    $stockProductoActualizado = $_POST['stockProducto'];
    $precioProductoActualizado = $_POST['precioProducto'];
    $descripcionProductoActualizada = $_POST['descripcionProducto'];

    $consultaActualizacion = "UPDATE productos SET product_nombre = ?, product_marca = ?, product_categoria = ?, product_stock = ?, product_precio = ?, product_descripcion = ? WHERE product_id = ?";
    $stmtActualizacion = mysqli_prepare($conexion, $consultaActualizacion);
    mysqli_stmt_bind_param($stmtActualizacion, 'sssidii', $nombreProductoActualizado, $marcaProductoActualizada, $categoriaProductoActualizada, $stockProductoActualizado, $precioProductoActualizado, $descripcionProductoActualizada, $IDproductoActualizado);
    $resultadoActualizacion = mysqli_stmt_execute($stmtActualizacion);

    if ($resultadoActualizacion) {
        echo "<p>Producto actualizado exitosamente.</p>";
        echo "<a class='buttonvolver' href='./6 - index - admin - productos.php'>Volver a la lista de productos</a>";
    } else {
        echo "<p>Error al actualizar el producto: " . mysqli_error($conexion) . "</p>";
    }
}
